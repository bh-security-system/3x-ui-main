...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/bh-security-system/3x-ui-main/master/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/bh-security-system/3x-ui-main/master/raw/main/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
